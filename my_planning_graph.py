...
class PlanningGraph():
        ...
        for a in self.all_actions:
            ...
            if pg_node.prenodes.issubset(self.s_levels[level]):
                ...
                for s_level in self.s_levels[level]:
                    s_level.children.add(pg_node)
        self.a_levels.insert(level, pg_nodes)
    ...
    def add_literal_level(self, level):
        ...
        # the new S nodes as children of all the A nodes that could produce
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # through set minus operation check if node_a2 has any negative effects
        # that would remove an effect from node_a1 or vice versa
        a1_diff = a1_add - a2_rem
        a2_diff = a2_add - a1_rem

        # check for inconsistencies
        if len(a1_add) > len(a1_diff) or len(a2_add) > len(a2_diff):
            return True
        else:
            return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # check all possibilities where an added or removed effect is in the
        # set of its inverse creating a cancellation
        if (a1_effect_add.intersection(a2_precond_neg) or
            a1_effect_rem.intersection(a2_precond_pos) or
            a2_effect_add.intersection(a1_precond_neg) or
            a2_effect_rem.intersection(a1_precond_pos)):
            return True
        else:
            return False
